...
def main(argv):
	...
	p.add_option('--cytomine_proba_threshold', type = 'float', dest = 'cytomine_proba_threshold', help = "probability threshold above which the annotation is classified as positive")
	...
	parameters['cytomine_proba_threshold'] = options.cytomine_proba_threshold
	...
	if parameters['path_to_project_info'] is not None:
		path = os.path.join(parameters["path_to_project_info"], str(parameters["cytomine_id_project"]))
		df = pd.read_csv("{}/image_info.csv".format(path), sep = ';')
		images_info = df.loc[df['Term ID'] == 20202].as_matrix(['Image ID', 'Job user ID'])

	else:
		images_info = np.array([parameters["cytomine_id_image"], parameters['cytomine_id_userjob']])
	...
	for image_id, user_job_id in images_info:
		if np.isnan(user_job_id):
			continue

		user_job_id = int(user_job_id)
		image_id = int(image_id)

		candidate_annotations = conn.get_annotations(id_user = user_job_id,
													 id_image =image_id,
		...
		file = open(os.path.join(folder_name, "predictions.txt"), 'w')
		file.write("PROJECT : {}\nIMAGE : {}\nUSERJOB : {}\n\n\n".format(parameters["cytomine_id_project"], image_id, user_job_id))
		...
		for i, annotation in enumerate(candidate_annotations.data()):
			...
			conn.fetch_url_into_file(url, filename, False, False)
		...
		for k, annotation in enumerate(candidate_annotations.data()) :
			...
			file.write("Annotation filename %s id: %d class: %d proba: %f\n" %(filename,annotation.id,int(y_predict),y_rate))
			print "Annotation filename %s id: %d class: %d proba: %f" %(filename,annotation.id,int(y_predict),y_rate)


			if y_rate > parameters["cytomine_proba_threshold"]:
				new_annotation = conn.add_annotation(annotation.location, image_id)
				conn.add_annotation_term(new_annotation.id, int(y_predict), int(y_predict), y_rate, annotation_term_model = models.AlgoAnnotationTerm)
